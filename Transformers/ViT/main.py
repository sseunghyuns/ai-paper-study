...
def parse_args():
    ...
    parser.add_argument('--latent_vector_size', default=128, type=int, help='latent vector size') # D
...
def run(args):
    ...
    hidden_dim = int(latent_dim/2) # Encoder 내 MLP의 hidden layer 노드 수 
    num_patches = int((args.img_size * args.img_size) / (args.patch_size * args.patch_size)) # HW/P^2
    patch_vec_size = ((args.patch_size**2)*3) # P^2 * C
    device = args.device
    ...
    # Load ViT 
    model = ViT(patch_vec_size=patch_vec_size,
    ...
    if args.mode == 'train' :
        ...
        model.eval()
        test_accuracy, test_loss = evaluation(model, test_loader, criterion, device)
